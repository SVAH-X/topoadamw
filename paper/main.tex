...
\paragraph{Model.}  We use CifarNet, a five-block convolutional network
(Conv $\to$ BatchNorm $\to$ ReLU $\to$ MaxPool, followed by a two-layer MLP
classifier) with approximately \todo{X}M parameters.
...
All experiments run on \todo{hardware spec}.  \todo{Report mean $\pm$ std over
N seeds.}
...
\todo{The following ablations should be run and reported.}

\paragraph{Probe density.}  Compare $n_s \in \{0, 4, 8, 16, 32\}$ random
samples to assess sensitivity of the variance estimate.

\paragraph{Probe interval.}  Compare $T \in \{10, 25, 50, 100, 200\}$ to
study the trade-off between overhead and adaptation frequency.

\paragraph{Heuristic thresholds.}  Vary the sharpness and variance thresholds
in Eq.~\eqref{eq:heuristic} to evaluate robustness.

\paragraph{TDA bootstrapping speed.}  Compare \texttt{min\_samples}
$\in \{50, 100, 200\}$ for the TopoCNN warmup to assess how quickly the CNN
